...
export default class Crossword{
    ...
    checkWin(){
        if(this.checkFilled())
        {
            this.checkGrid(null, false);
            for(let i = 0; i < this.numrows; i++)
            {
                if(this.checkedBoard[i].includes("F")){
                    return false;
                }
            }
    ...
    revealGrid(){
        ...
        for(let i = 0; i < this.answers_across.length; i++){
            this.revealedWords.push(this.answers_across.length[i]);
        }
        for(let i = 0; i < this.answers_down.length; i++){
            this.revealedWords.push(this.answers_down.length[i]);
        }
        return this.puzzleBoard;
    }
    ...
    */
    ...
    unpause(){
        ...
        this.totalPauseTime += this.timePaused - unpause_time;
        this.timePaused = null;
    }

    gameFinish(){
        let gameEndInfo = [];
        gameEndInfo.push(this.getScore());
        return gameEndInfo;
    }
    ...
    getScore(currentBoardState){
        if(this.completed){
            ...
            if(this.revealed){
                ...
                for(let i = 0; i <= this.numcols; i++){
                    let rowCount = this.revealedTiles[i].reduce(function(accumulator, currentVal){
                        if(currentVal === "true"){
